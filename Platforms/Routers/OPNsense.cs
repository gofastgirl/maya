...
{
    public class OPNsense : ISystems
    {
        ...
        public static async Task<SystemDetails> CreateSystemDetails(string ip)
        {
            ...
            s.PlatformIdentifier = new PlatformIdentifier(EnumPlatformIdentifier.OPNSENSE);
        ...
        /*  public static async Task<SystemDetails> ScanThisSystem(string IP4Address)
         {
             Logger.ShowFunction();
             // Simulate scanning logic for OPNsense
             Logger.Info($"Scanning OPNsense at {IP4Address}...");
             // Return a dummy SystemDetails object for demonstration purposes
             SystemDetails s = new SystemDetails(IP4Address, 8006, "OPNsense");
        ...
        {
            Logger.ShowFunction("OPNsense", "ScanThisSystem");
            // Simulate scanning logic for OPNsense with existing details
            Logger.Info($"Scanning OPNsense with details: {details.UniqueName}...");
            // get data from OPNsense
            string json;
            List<OPNsenseMachine> allmachines = new List<OPNsenseMachine>();
            List<OPNsenseMachine>? tempmachines = new List<OPNsenseMachine>();
            var authenticator = new OpnsenseAuthenticator(
                Secrets.OPNSENSE_USER,
                Secrets.OPNSENSE_PASSWORD
            );
            ...
            )
            {
                ...
                {
                    Logger.Error("Error during OPNsense scan: " + ex.Message);
                    return;
                }
                using (var doc = JsonDocument.Parse(json))
                {
                    ...
                    tempmachines = JsonSerializer.Deserialize<List<OPNsenseMachine>>(
                        dataElement.GetRawText()
                    );
                    if (tempmachines != null)
                    {
                        foreach (OPNsenseMachine pm in tempmachines)
                        {
                            allmachines.Add(pm);
                        }
                    }
                ...
                foreach (OPNsenseMachine vm in allmachines)
                {
                    ...
                    thisvm.Functions.Add(new SystemFunction(EnumSystemFunction.DHCP));
                    thisvm.SystemRole = new SystemRole(EnumSystemRole.ROUTER);
                    thisvm.SystemName = new SystemName("");
                    thisvm.OsType = new OsType(EnumOsType.ALPINE);
                    ...
                    thisvm.PlatformIdentifier = new PlatformIdentifier(
                        EnumPlatformIdentifier.OPNSENSE
                    );
        ...
        public static async Task<bool> IsCorrectSystem(SystemDetails details)
        {
            ...
            )
            {
                ...
                {
                    { "username", Secrets.OPNSENSE_USER },
                    { "password", Secrets.OPNSENSE_PASSWORD },
                };
                ...
                /*  string url =
                 ...
                 return ticket != null;
    */
                return true; // Temporarily bypassing actual check for testing
            }
        }
